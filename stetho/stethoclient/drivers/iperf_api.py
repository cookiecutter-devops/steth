...
class CheckIperf(Lister):
    ...
    def take_action(self, parsed_args):
        ...
        res = server.setup_iperf_server(
            protocol=parsed_args.server_protocol, port=parsed_args.server_port
        )
        ...
        if res["code"] == 1:
            ...
            sys.exit()
        if res["code"] == 0:
            msg = ("Iperf server setup success and runs in " "pid:%s") % (
                res["data"]["pid"]
            )
            Logger.log_high(msg)
        ...
        if r["code"] == 1:
            ...
            sys.exit()
        if r["code"] == 0:
            msg = ("Iperf server delete success and " "pid:%s") % (iperf_server_pid)
            Logger.log_high(msg)
